...
(() => {
  ...
  const VERSION = 'v0.22.9 Multiplayer Alpha · M10';
  ...
  function roomReady() {
    ...
    return humanSeats().length > 0 && seats.every(s => occupied(s) && (s.kind === 'cpu' || s.connected) && Number.isInteger(s.roll) && s.roll >= 1 && s.roll <= seats.length) && new Set(seats.map(s => s.roll)).size === seats.length;
  }
  ...
  function renderRoom() {
    ...
    dialog.innerHTML = '<div class="eyebrow">Multiplayer lobby · ' + (isHost ? 'you are host' : 'connected guest') + '</div><h2>Choose your Traveler</h2><div class="room-code"><span>Room code</span><b>' + safe(roomCode(room.id)) + '</b><button id="mpCopyCode">Copy code</button></div>'+relayMarkup() +
      ...
      '<div class="mp-seat-list">' + seats.map(renderLobbySeat).join('') + '</div>' + (isHost ? '<button class="mp-fill-cpu" id="mpFillCPU">Fill open slots with CPU</button>' : '') + '<div class="mp-room-footer"><div class="mp-room-status">' + (roomReady() ? 'Starting order locked: ' + order.map((s,i) => (i+1)+'. '+safe(s.customName)+' (position '+s.roll+')').join(' · ') : 'Pre-game roll-off: each roll permanently claims one unused position. Available: '+(availablePositions.length?availablePositions.join(', '):'none')+'. Highest position acts first; ties are impossible.') + '</div><button class="primary mp-start" id="mpStart" ' + (!isHost || !roomReady()?'disabled':'') + '>Begin the Crossing</button></div><div class="mp-alert" id="mpLobbyAlert"></div>' + engineConfigMarkup();
