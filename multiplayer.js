...
(() => {
  ...
  const VERSION = 'v0.22.2 Multiplayer Alpha · M3';
  ...
  function joinRoom(hostId) {
    ...
    ensurePeer(null, () => {
      ...
      joinTimeout = setTimeout(() => {
        ...
        renderJoin('The room was found, but no WebRTC path opened. '+(relayState === 'ready' ? 'The TURN relay was available; ask the host to reload the same M3 build.' : 'TURN relay fallback is not available on this deployment.'));
      }, 15000);
    });
  }
  ...
  function handleLobbyAction(sender, action, payload = {}) {
    ...
    if (action === 'capacity' && sender === room.hostToken) {
      ...
      if (!roomReady()) return sendNotice(sender, 'Assign every active slot to a connected Human or CPU, keep at least one Human, and finish initiative.');
      startNetworkGame();
      return;
    }
    broadcastLobby();
  ...
  function resetInitiative() { room.seats.forEach(s => s.roll = null); room.tieRound = 0; scheduleCPURolls(); }
  ...
  function scheduleCPURolls() {
    ...
    setTimeout(() => {
      ...
      seat.roll = 1 + Math.floor(Math.random() * 6);
      room.rolling = null;
      pushSystem('CPU ' + seat.customName + ' rolled ' + seat.roll + ' for initiative.');
      resolveInitiativeTies();
      broadcastLobby();
      scheduleCPURolls();
    }, 780);
  ...
  function resolveInitiativeTies() {
    ...
    if (!seats.length || seats.some(s => s.roll === null)) return;
  ...
  function roomReady() {
    ...
    return humanSeats().length > 0 && seats.every(s => occupied(s) && (s.kind === 'cpu' || s.connected) && Number.isInteger(s.roll)) && new Set(seats.map(s => s.roll)).size === seats.length;
  }
  ...
  function renderRoom() {
    ...
    dialog.innerHTML = '<div class="eyebrow">Multiplayer lobby · ' + (isHost ? 'you are host' : 'connected guest') + '</div><h2>Choose your Traveler</h2><div class="room-code"><span>Room code</span><b>' + safe(roomCode(room.id)) + '</b><button id="mpCopyCode">Copy code</button></div>'+relayMarkup() +
      ...
      '<div class="mp-seat-list">' + seats.map(renderLobbySeat).join('') + '</div>' + (isHost ? '<button class="mp-fill-cpu" id="mpFillCPU">Fill open slots with CPU</button>' : '') + '<div class="mp-room-footer"><div class="mp-room-status">' + (roomReady() ? 'Initiative locked: ' + order.map((s,i) => (i+1)+'. '+safe(s.customName)+' ('+s.roll+')').join(' · ') : 'Assign every slot to a Human or CPU. Humans roll their own die; CPU rolls and tie rerolls happen automatically. Highest acts first.') + '</div><button class="primary mp-start" id="mpStart" ' + (!isHost || !roomReady()?'disabled':'') + '>Begin the Crossing</button></div><div class="mp-alert" id="mpLobbyAlert"></div>' + engineConfigMarkup();
    ...
    if (isHost) document.getElementById('mpFillCPU').onclick = () => lobbyAction('fillCPU');
  ...
  function renderLobbySeat(seat) {
    ...
    const roll = seat.roll === null ? (cpu ? '<small>'+(room.rolling===seat.slot?'CPU rolling…':'CPU automatic')+'</small>' : mine ? '<button data-roll="'+seat.slot+'" '+(room.rolling?'disabled':'')+'>'+(room.rolling===seat.slot?'Rolling…':'Roll 1–6')+'</button>' : '<small>'+(open?'Unassigned':'Awaiting roll')+'</small>') : '<div class="initiative-die"><span>'+seat.roll+'</span></div>';
